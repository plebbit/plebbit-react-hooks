...
describe('feeds', () => {
  ...
  describe('get feed', () => {
    ...
    test(`subplebbit updates while we are scrolling`, async () => {
      ...
      try {await rendered.waitFor(() => rendered.result.current.bufferedFeed[0].cid === 'updated sorted posts cid topAll comment cid 100')} catch (e) {console.error(e)}
    ...
    describe('getSortedPosts only gets called once per sortedPostsCid', () => {
      ...
      test(`store sorted posts pages in database`, async () => {
        ...
        const rendered2 = renderHook<any, any>(() => useFeed(['subplebbit address 1'], 'new'), { wrapper: PlebbitProvider })
        try {await rendered2.waitFor(() => rendered2.result.current.feed?.length >= postsPerPage)} catch (e) {console.error(e)}
