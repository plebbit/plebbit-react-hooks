...
describe('mock content', () => {
  ...
  test('use comments', async () => {
    ...
    try {await rendered.waitFor(() => typeof rendered.result.current.upvoteCount === 'number')} catch (e) {console.error(e)}
    console.log(rendered.result.current)
  ...
  test('use feed', async () => {
    const rendered = renderHook<any, any>((subplebbitAddresses) => useFeed(subplebbitAddresses, 'new'), { wrapper: PlebbitProvider })
