...
for (const plebbitOptionsType in plebbitOptionsTypes) {
  describe(`accounts (${plebbitOptionsType})`, () => {
    ...
    describe(`create subplebbit (${plebbitOptionsType})`, () => {
      ...
      // TODO: this test cannot work because there's no way to mock the subplebbit captcha and update interval
      it.skip('edit a subplebbit settings and wait for subplebbit update', async () => {
        console.log('before create subplebbit')
        const createdSubplebbitTitle = 'my title'
        let subplebbit
        await act(async () => {
        ...
        console.log('before edit subplebbit')
        const subplebbitSettings = {features: {authorFlairs: true}}
        let onChallenge = () => {}
        let onChallengeVerification = () => {}
        await act(async () => {
          await rendered.result.current.publishSubplebbitEdit(createdSubplebbitAddress, {...subplebbitSettings, onChallenge, onChallengeVerification})
        })
        console.log('after edit subplebbit')

        console.log('before reset')
        rendered.rerender(`doesn't exist`)
        await waitFor(() => !rendered.result.current.subplebbit)
        console.log('after reset')

        console.log('before use subplebbit')
        rendered.rerender(createdSubplebbitAddress)
        await waitFor(() => rendered.result.current.subplebbit.address === createdSubplebbitAddress)
        expect(rendered.result.current.subplebbit.address).to.equal(createdSubplebbitAddress)
        expect(rendered.result.current.subplebbit.title).to.equal(createdSubplebbitTitle)
        expect(rendered.result.current.subplebbit.features).to.deep.equal(subplebbitSettings.features)
        const subplebbitUpdatedAt = rendered.result.current.subplebbit.updatedAt
        console.log('after use subplebbit')

        // trigger subplebbit update
        await act(async () => {
          console.log('before publishComment')
          await rendered.result.current.publishComment({
            subplebbitAddress: createdSubplebbitAddress,
            onChallenge,
            onChallengeVerification,
            title: 'some title',
        ...
        await waitFor(() => {
          console.log(Date.now(), rendered.result.current.subplebbit.updatedAt)
          return rendered.result.current.subplebbit.updatedAt !== subplebbitUpdatedAt
        })
        console.log(rendered.result.current.subplebbit)
        console.log('subplebbit updated')
      })
    })
